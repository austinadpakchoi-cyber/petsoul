# PetSoul — 代理协作约定（Codex / Claude Code 共用）

所有 AI 编码代理（Codex、Claude Code 等）在本仓库工作前必须读完本文件。规则只在这里维护一份，`CLAUDE.md` 指向本文件。


## 当前任务入口：96 小时网页重建

当前用户方向：手机网页优先、香港部署、完整注册、上传自己的宠物或领养伙伴；共同的家成为首页，主人守家、种菜和互偷支持宠物旅行。宠物在这个世界永远不会受伤，参军/飞行员等属于酷炫冒险表达，可产生 AI 照片。旅行可带回稀有种子与物资，交易市场、真实原型公益领养、原创影视 IP 和文旅是发展方向；游戏金币可用于世界内公益活动，不等同现实捐款。96h 首发取舍仍按产品文档与用户决定。下方旧 iOS 规则用于维护旧工程，不能当作网页的全部产品要求。

已确认的归属规则：所有宠物均为专属，包括用户上传、真实原型领养和原创领养伙伴；同一只宠物只归属一个家庭，领养后不得重复发给其他用户，不采用多人共同领养或同角色复制发放。具体见产品方案。

2026-09-22 用户新决定（追加，优先于上一句中与之冲突的部分，原文保留作历史）：一只宠物仍只属于一个家庭，但一个家庭可以有多位成员共同照顾（家庭管理员 / 共同照顾者；妈妈、哥哥这类称呼只是称呼，不带权限），也可以有多只宠物共用一个家（家园、菜园、家庭仓库共用；每只宠物各自的身份、DNA、星球账户、证件、驾照、旅程与和每位家人的私聊）。“不采用多人共同领养”从此只指不把同一只宠物同时发给多个家庭，不再表示不允许家人共同照顾。注册不强制建宠物：访客可以不登录先逛逛；三种入口（先逛逛 / 带我的宠物来 / 认识新伙伴）加家庭邀请直达。待领养居民领养前就在星球上生活（稳定 pet_id、住星球居民驿站、不编造人类主人），领养后身份与经历连续、在外面时不瞬移。详见 `docs/product/PETSOUL-2.0-MASTER-PLAN.md` 的“2026-09-22 修订”与 `docs/coordination/FRONTEND-HANDOFF.md`。

各窗口任务由用户分配，不按模型名称预设职责，不自行创建工作目录或 worktree。统一启动提示词见 `docs/coordination/WINDOW-START-PROMPT.md`。

新增产品特色：`docs/product/FOOD-DISCOVERY-ENGINE.md` 定义寻味引擎，宠物与主人双偏好、分店/菜品证据和事实约束的推荐。框架先预留模块/契约/fixture，后续业务由用户安排；虚拟探店和 AI 照片不得当作真实餐厅品质证据。总方案当前 v1.5，旧选店分不等于寻味品质分。

交通与同行规则见 `docs/product/TRANSPORT-COMPANION-SYSTEM.md`：真实路线/时长、独立稳定动物世界承运人及班次；首版默认核验时刻表，道路标预计。交通与媒体时钟分离，途中可一起听看，到站保存进度，驾驶禁视频。R0 同时预留 Transport/CompanionMedia 和寻味的到达时间契约；模型记忆不是已核验班次、外部影音链接不是同步播放。不能为现场展示压缩正式交通时间。

用户已明确交通 UI：汽车/飞机/火车/轮船在地图上正常移动，交通工具旁音符/小电视表示当前活动，点击打开相应音乐/视频播放 UI；不做独立舱室页面。框架须预留地图车辆、活动图标和播放面板接口，不能恢复已撤销的空间式设计。

注册接待见 `docs/product/RECEPTION-ONBOARDING-MEMORY.md`：AI 接待员听主人交代，整理可编辑叮嘱，确认用途后再进入角色记忆。倾诉可跳过，私人记录检索前隔离，不用失忆威胁促披露；保存、转达与实际应用分别留状态。R0 预留 reception / MemoryPolicy / HomeWelcome，第一次入住体现一个支持的细节；旧 create_pet 立即启动旅行的路径需由网页流程适配。

意图层见 `docs/product/INTENT-DECISION-LAYER.md`：Jev 仅为可替换候选，优先接待/通讯，默认关闭新增远端调用。R0 只加多信号契约/开关/离线样例；中文、香港耗时与全链路成本未验证，不阻塞现有框架。识别不代替权限、确认、真实交通时间或执行；旧关键词/字符取模规则不能作高影响动作的安全后备。私人限制覆盖回复、事件、trace、记忆及衍生物，不能只替换分类标签。已开工窗口补记增量，不重初始化、不重置 96h，不替用户分配后续任务。

开工前读取所有 `docs/coordination/WINDOW-*.log.md`，登记任务、Git 基线、修改范围与运行资源。每个窗口只追加自己的记录；未明确释放的范围不能自行接管。冲突只暂停重叠部分。

同一工作副本共享分支、暂存区和文件。拉取、切分支、提交、合并、清理等整仓操作须由用户指定窗口在协调后执行，不能自行切分支来“隔离”窗口。普通已授权本地开发直接执行。

产品与工程总入口为 `docs/product/PETSOUL-2.0-MASTER-PLAN.md`。用户已指定 `claude-20260922-014933-307b` 先搭整体框架，任务见 `docs/coordination/CLAUDE-FOUNDATION-PROMPT.md`；其他模块仍由用户分配。总方案优先于旧专题中的阶段建议；`WEB-REBUILD-BRIEF.md`、`96H-PLAN.md`、`MAP-VENUE-SOCIAL.md` 保留细节。计划和框架存在都不等于功能或部署已完成。已核对的旧提交没有网页客户端，但共享工作树已出现 Claude 在建的 PetJourneyWeb，进度以日志和交接证据为准；下方历史 `petjourney-mvp/` 描述不代表当前目录实际存在。

## 项目是什么

PetSoul / PetJourney 是"宠物在平行世界继续生活"的**关系型**旅行产品。核心目标：让已去世宠物的主人感到 TA 仍然存在，并实现"宠物替主人去旅行"的替代自由感。**这不是 AI 陪聊工具**——所有功能决策都要服务于"TA 有自己的生活、路线、节奏，用户温柔地参与"，而不是即时应答的客服感。

产品模块：地图（开车倾向第一人称导航视角）、通讯器（微信式，按宠物真实状态决定马上回/晚点回/发位置/发照片）、朋友圈/旅途圈（宠物自发的轻量动态）、明信片（正式、私密、收藏级纪念）、回忆（"TA 确实生活过"的证据）、证件卡包（PetSoul 世界身份系统，护照只在跨境远行时出现）、世界杯球场彩蛋（地图上点亮美加墨球场，不放硬按钮）。

## 硬规则（违反即返工）

1. **UI 代码不允许写死"小福"或任何特定宠物的素材。** 小福只是测试样板宠物，只能出现在测试/Mock 数据里（`MockPetJourneyService.swift`）。所有界面围绕"当前宠物"（名字、照片、DNA、性格、旅行状态）动态生成。
2. **文案不得暴露内部机制**（如"第一版会做模拟识别"），保持仪式感。
3. **iOS 工程是经典 pbxproj**（objectVersion 56）：新建 Swift 文件必须同步登记进 `project.pbxproj`，否则 Xcode 不收录。**鼓励按职责拆分文件**——新建文件是正常且被鼓励的，只要登记 pbxproj 即可；不要为了省登记步骤而往现有文件里无限堆代码。单文件宜控制在 ~800 行以内、单一文件内类型不宜超过 ~30 个，超过就应拆分（可跑 `scripts/arch_gate.py` 自查）。
4. **改动 iOS 代码后必须构建/测试通过**（命令见下）。

## 仓库结构

- `PetJourneyIOS/` — SwiftUI 主 App（约 2.3 万行）。大文件：`JourneyMapView.swift`（地图+世界杯）、`CommunicatorViews.swift`（Tab 结构+通讯器+朋友圈+回忆+卡包）、`SecondaryViews.swift`（攻略/明信片/纪念品/DNA）。**避免多个代理同时编辑这几个大文件。**
- `PetJourneyBackend/` — Python FastAPI 世界模拟引擎（pet_life_engine、travel_quest_engine、route_planner、photo_pipeline 等），`tests/` 含 LLM 评测用例。
- `petjourney-mvp/` — 早期 Next.js 网页 MVP，已被 iOS 取代，不要在里面开发新功能。
- `audits/` — UX 审计记录。

服务模式：`AppSessionStore` 默认 remote，指向生产 `https://api.petsoul.games`（Release 与 DEBUG 真机一致，TestFlight 包不会落到 Mock）；测试环境默认 Mock；DEBUG 下可用 `PETJOURNEY_BASE_URL` 环境变量把真机指向本机后端联调。

## 架构分层（2026-08-26 起生效，配合 scripts/arch_gate.py 机器检查）

**后端依赖方向**（自上而下，底层不得反向 import 上层）：

```
routers → engines（agent_engine / communicator / pet_life_engine / world_simulation
        / travel_quest_engine / image_provider 等包）→ repositories → storage/infra
schemas / utils / config 是公共底层：只允许 import schemas、标准库与底层工具，
不得 import engines 或 routers。组合根（main.py / dependencies.py）可以 import 一切。
```

- 新增引擎的形态规则：**单文件 ≤ 400 行且职责单一 → 平铺 `app/` 根目录；超过其一 → 建包**（`app/<name>/` 多文件），并参照 `app/storage.py` 的门面模式：`app/<name>.py` 保留历史导入面、注释交代拆分原因。
- 工具模块（如 `http_utils.py`）不得 import 引擎层；领域解析归 `schemas`，路由辅助归 `routers`/`dependencies`，演示数据播种归启动路径。
- **世界模拟的墙上时间（钟点）一律走 `app/city_timezones.py`，跟随「TA 所在城市」，禁止裸 `now.astimezone()` / `date.today()` 取宿主机时区**（CI 与生产都是 UTC，宿主机时区会让 TA 的活动、用餐、睡眠整体漂移；`meal_rules` 约定：无时区 datetime 表示墙上时间本身，带时区的 instant 需传城市名换算）。

**iOS 类型归属**：

- `Views/` **只放 View 类型**：SwiftUI View（含 `UIViewRepresentable`/`UIViewControllerRepresentable`）与 UIKit 视图子类（`UIControl`/`MKAnnotationView` 等）。领域模型、枚举、构建器、布局几何辅助一律进 `Models/`（或新建 `Presentation/`）。
- ViewModel 进 `ViewModels/`，一个 ViewModel 一个职责面；**跨文件 extension 拆分不能替代职责拆分**——`arch_gate.py` 已按类型名跨文件归并体量（阈值 600 行 / 25 定义），超标的「待拆分」清单会出现在门禁输出里，拆分后自动消失。
- 新文件照旧必须 `scripts/register_swift_file.py` 登记 pbxproj；迁移文件 = 移动 + 登记，不要顺手改逻辑。

## iOS 设计系统（DesignTokens.swift）

- 全部颜色通过 `Color(light:dark:)` 自适应：日间瓷白，夜间是"灯下暖夜"（深暖绿炭底），不是纯黑。
- 语义令牌：`surface`/`surfaceStroke` 用于卡片与玻璃表面；`deepInk` 用于承载白色文字的实心填充（主按钮、Toast）和**所有阴影**（`ink` 在深色模式会变浅，当阴影用会发白光）。
- `paper`/`paperShade`/`paperInk`/`paperSecondaryInk`/`paperAccent` 是**固定色**：明信片等纸质纪念物是"实物"，夜里也保持暖纸配深墨。
- 原生 `.white` 只允许用于：彩色填充上的文字/图标、镜面高光（卡面闪光、shimmer）、实物材质（拍立得相框）、地图上照片头像的贴纸式白圈。**表面职责一律用语义令牌，禁止硬编码 `.white` 作背景/卡片。**
- 圆角体系：`cardRadius`(18) 必须大于 `controlRadius`(14)——卡片比控件圆。
- 常驻动画（`TimelineView` 类：AmbientSignalField、SignalPulseRings、SignalBars、EnergyRing）必须响应 `accessibilityReduceMotion`，帧率上限 1/30。
- "信号"是品牌母题：连接状态用信号语言表达（"已送达 TA 的世界"），不用即时通讯范式（"正在输入…"）。Tab 命名"通讯器"，不叫"手机"。

## 构建与测试

2026-09-23 后端运行层修订（追加，原文保留作历史）：网页后端的生活类读接口（`/home`、`/journey/map`、`/visits/{id}`、`/timeline`、`/jobs`、
`/communicator/{pet}/messages`、`/credentials`、`/collection`、`/circle/feed`）是**纯读**的：只按服务器时间投影画面，不推进世界、不调模型与地图；
世界的推进只发生在任务进程（世界线 + 认知线，各一份数据库租约）与命令里。到期结算（工资、世界事件、行程完成、投递回执）在同一个写事务里提交，
提交之后才按 outbox 投递家庭来信、动态、收藏、证件；攻略与相遇表达走认知线。新增后端代码要守住这三条：
读接口不写业务表、付费调用在发出前先原子预占、需要时间的结果提交时按领取令牌与语义版本复核。
配置：`PETJOURNEY_WEB_HEARTBEAT_MODE`（默认 shadow，只评估不执行）、`PETJOURNEY_WEB_BRAIN_MODE`（默认 off，一次模型都不调用）。
详见 `docs/coordination/FRONTEND-HANDOFF.md` 的 0.4.1 节与 `docs/contracts/MODULE-MAP.md`。

iOS（改动后必跑）：

```bash
cd PetJourneyIOS
xcodebuild -project PetJourneyIOS.xcodeproj -scheme PetJourneyIOS \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' test
```

注意：模拟器测试偶发抽风失败，失败先重跑一次再排查。

后端（用 unittest，不是 pytest；**必须带 `TZ=UTC`**，与 CI 对齐——宿主机时区相关 bug 只在 UTC 下暴露）：

```bash
cd PetJourneyBackend
pip install -r requirements.txt   # 首次
TZ=UTC /opt/miniconda3/bin/python3.12 -m unittest discover -s tests
```

## 协作方式

- 独立工作副本可使用各自分支；多窗口共用一个工作副本时遵循上方留痕与整仓操作规则，不自行切分支。合并前跑对应测试。
- 分工建议按模块/目录切，尤其不要同时改 `CommunicatorViews.swift`、`JourneyMapView.swift`、`SecondaryViews.swift` 这三个大文件。
- 对世界观/文案有疑问时，宁可保守：不确定的机制描述不要写进用户可见文案。
- 分支清理（2026-08-26 起为硬规则）：每一个完成合入 main 的分支必须及时清理——本地 `git branch -d <branch>` 与远端 `git push origin --delete <branch>` 同时删除（用 `-d` 而非 `-D`，它会在分支未完全合入时拒绝删除，是安全网；若 `-d` 报错先查清楚，不要改用 `-D`）。定期自查 `git branch --merged main` 与 `git branch -r --merged main` 应为空。
