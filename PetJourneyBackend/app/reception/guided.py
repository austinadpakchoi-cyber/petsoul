...
MODEL_DISCLOSURE = "我是 PetSoul 的 AI 接待角色。你选择了让我用对话模型（{provider}）回应：你在这次接待里写的话会发送给该服务商生成回应；便笺仍然只摘录你的原话，由你决定交给 TA、只留在这里还是不保存。随时可以跳过。"
HOST_DISCLOSURE = "我是 PetSoul 的 AI 接待角色，现在是引导便笺模式：你写的话会原样放进便笺，由你决定交给 TA、只留在这里还是不保存。不想说随时可以跳过。"
...
FOLLOW_UPS = [
    "我记下了，放进待确认的便笺。还有别的小习惯吗？比如喜欢或不喜欢的互动。",
    "谢谢你。它有没有特别熟悉的物件，或者一个只有你们懂的称呼？",
    "好的。想说的差不多了就可以去整理便笺；以后想起来，还能从家里或通讯器再补。",
]
...
def model_messages(branch: ReceptionBranch, pet_name: str, turns: list[dict]) -> list[dict[str, str]]:
    ...
    system = "\n".join((
        ...
        "不要说“记下了/记住了/已保存/放进清单”——便笺要由主人确认后才保存；也不要承诺系统或宠物会做什么具体安排；",
