...
const SCENES: { scene: PhotoScene; title: string; kicker: string; detail: string; fiction: boolean }[] = [
  ...
  { scene: "flight_adventure", title: "想象一次飞行冒险", kicker: "03 / 虚构主题", detail: "明确是幻想照片；不写入真实旅程、不扣旅费、不发勋章。", fiction: true },
];
...
export function PhotoRequestsPage() {
  ...
  const photoPermission = useQuery({ queryKey: queryKeys.householdDetail(userId ?? "-", householdId), queryFn: ({ signal }) => households.detail(householdId, signal), enabled: Boolean(userId && householdId) });
  ...
  const list = useQuery({ queryKey: queryKeys.photoRequestsFor(userId ?? "-", petId), queryFn: ({ signal }) => pets.photoRequests(petId, signal), enabled: Boolean(userId && petId) });
  ...
  return <Page className="ps-photo-page"><TopBar title={`${pet?.name ?? "TA"} 的照片`} subtitle="申请、结果和重画都在这里" back="/home" /><div className="ps-photo-content">
    <header className="ps-photo-hero"><span>PETSOUL · A MOMENT OF LIFE</span><h1>留住 TA<br />正在生活的一刻</h1><p>普通生活与想象中的冒险，分开记录。</p></header>
    ...
    <section className="ps-photo-list"><div className="ps-photo-list__title"><div><span>ALBUM / 照片结果</span><h2>申请记录</h2></div><button type="button" onClick={() => void list.refetch()} disabled={list.isFetching}>{list.isFetching ? "正在刷新…" : "刷新结果"}</button></div>{visibleRetryNote ? <p role="status" className="ps-photo-feedback">{visibleRetryNote}</p> : null}{retry.isError && retryInScope ? <p role="alert" className="ps-photo-feedback">{"还没确认这次重画的结果。请刷新列表查看，或稍后再试。"}</p> : null}{list.isPending ? <LoadingState label="正在读取照片结果…" /> : list.isError ? <ErrorState error={list.error} onRetry={() => void list.refetch()} /> : list.data?.length ? list.data.map((item) => <PhotoRequestCard key={item.request_id} item={item} retrying={Boolean(retryInScope && retry.isPending && retry.variables?.requestId === item.request_id)} onRetry={(id) => { if (!userId || !petId) return; setRetryNote(null); retry.mutate({ userId, petId, requestId: id }); }} />) : <EmptyState icon="camera" title="还没有照片结果">拍下的照片会出现在这里。暂时拍不了的场景不会留下空白记录。</EmptyState>}</section>
  </div></Page>;
}
