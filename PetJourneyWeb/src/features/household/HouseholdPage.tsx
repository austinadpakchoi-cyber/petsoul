...
function FamilyContent({ detail, invites, reload }: { detail: HouseholdDetail; invites: HouseholdInvite[]; reload: () => void }) {
  ...
  const saveRelationship = useMutation({ mutationFn: () => households.saveRelationship(pet!.pet_id, { owner_title: ownerTitle.trim() || null, relation_label: relationLabel.trim() || null }), onSuccess: () => void queryClient.invalidateQueries({ queryKey: queryKeys.householdRelationship(userId ?? "-", pet?.pet_id ?? "-") }) });
...
export function HouseholdPage() {
  ...
  const detail = useQuery({ queryKey: queryKeys.householdDetail(userId ?? "-", householdId), queryFn: ({ signal }) => households.detail(householdId, signal), enabled: Boolean(userId && householdId) });
  ...
  return <Page className="ps-family-page"><TopBar title="我们的家" subtitle="同一个家，彼此有各自的角色" back="/home" />{!householdId ? <EmptyState icon="home" title="还没有家庭">先让伙伴入住，家会出现在这里。</EmptyState> : detail.isPending ? <LoadingState label="正在翻开家庭档案…" /> : detail.isError ? <ErrorState error={detail.error} onRetry={() => void detail.refetch()} /> : detail.data ? <FamilyContent key={`${userId}:${householdId}`} detail={detail.data} invites={invites.data ?? []} reload={() => void detail.refetch()} /> : null}</Page>;
}
