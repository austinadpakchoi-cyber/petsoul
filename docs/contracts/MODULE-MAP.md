...
迁移编号区间：每个模块只在自己的区间建文件；`0001–0099` 平台共享（框架窗口）。已用：0001、0010、0020、0100、0150、0160、0200、0210、0220、0300、0400、0410、0450、0460、0500、0600、0650、0660、0670、0700、0800、0900、1000、1020、1100、1110、1120、1200、1210、1300、1400、1410、1420、1430。`1300–1399` 为宠物自主世界（`app/web_agent/`），`1400–1499` 为证件与驾考（`app/web_credentials/`、`app/web_driving/`），`1500–1599` 为员工运营后台（`app/web_admin/`），**`1600–1699` 为世界角色资产（`app/web_character/`，CR-PLAYER-CHARACTER-01）**。**`1800–1899` 为可领养的原创星球居民（首批 6 位 `m1800_first_residents`，I，2026-09-24）**，**`1900–1999` 为到站欢迎（`app/web_arrival/`，I，2026-09-24）**；身份区间另用 0190（模型回信默认开启的旧库补正，I，2026-09-24）。
...
| 可领养原创居民（I，2026-09-24） | —（领养卡照片 `photo_url` 由 6c2b 渲染） | **I 持有**：迁移 `m18xx_*`、`app/web_pets/platform_photos/`（打包的原创形象） | **1800–1899** | 首批 6 位无主原创居民（3 猫 3 狗）：候选、宠物、公开简介、驿站住处、已确认 DNA；形象照走公开路由 | residents（0240 的驿站与系统账号）、pets、dna | `python -m pytest tests/test_web_first_residents.py` | **已实现**：13 项＋100 subtests；无主居民的模型大脑未开启（需平台策略） | I |
| arrival 到站欢迎（I，2026-09-24） | 家庭频道带图消息与收藏明信片沿用现有渲染（到站专属文案与冲洗中占位交 6c2b） | **I 持有**：`app/web_arrival/**`、迁移 `m19xx_*`；入住接口里一行登记（`routers/web/identity.py`） | **1900–1999** | TA 第一次住进家后，认知线在家庭频道发「我到站啦」＋到站明信片，**同一张自拍**（一个生图任务两处回填）；生图不可用时寄手写明信片且不提自拍 | illustrations（`request_photo_in`）、collection（`postcard_in`）、communicator（`pet_note_in`：写给接 TA 入住那位家人的私聊，那位已不在则退回家庭频道）、home_places | `python -m pytest tests/test_web_arrival_selfie.py` | **已实现**：16 项；设计见 `docs/coordination/ARRIVAL-SELFIE-DESIGN-2026-09-24.md` | I |
| travel 路由（TRV-00 §23.4，I，2026-09-24） | `src/features/journey/**`、攻略心愿页与计划页（6c2b） | **I 持有**：`app/routers/web/travel.py`（DTO 在 `app/schemas/web/travel.py`） | —（纯读，无迁移） | `GET /travel/wish`（没有心愿回 200＋null）、`GET /travel/plans/{plan_id}`（每一版；别人家的与不存在一样 404）；只调 A 的只读视图，行程标价与用券由路由 join C 的 `journeys.repo` | web_travel（A 的 `wishes.read` / `plan_view_in`）、journeys（C） | `python -m pytest tests/test_web_travel_routes.py tests/test_web_travel_plan_route.py` | **已实现**：10 项；三条 POST（重研究、重画手账、取消心愿）未做 | I |
